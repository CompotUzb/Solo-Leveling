# Solo Leveling MVP Acceptance Runbook

This runbook starts and verifies the local-first Solo Leveling MVP. Complete the skip Discord flow first. Real Discord tracking is a separate, optional acceptance step.
...
```bash
git clone <REPOSITORY_URL> solo-leveling
cd solo-leveling
corepack enable
...
DATABASE_PATH=./data/solo-leveling.sqlite
API_HOST=127.0.0.1
API_PORT=3333
...
```bash
docker compose logs -f solo-leveling
```
...
```bash
docker compose restart solo-leveling
```
...
The comparison is `-ge` because the SSE verification quest may have added another 10 XP. Persistence is provided by the host file `./data/solo-leveling.sqlite`.
...
Stop the conflicting service before starting Solo Leveling.
...
```bash
docker compose ps
docker compose logs --tail=200 solo-leveling
```
...
Compose must mount `./data:/app/data`, and the database should exist at `./data/solo-leveling.sqlite`. Do not delete `./data` when testing persistence.
...
```bash
docker compose down
rm -f ./data/solo-leveling.sqlite ./data/solo-leveling.sqlite-shm ./data/solo-leveling.sqlite-wal
docker compose up -d
```
