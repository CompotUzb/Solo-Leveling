...
describe("Main Quest command handler", () => {
  ...
  it("answers help and unknown subcommands with usage text", async () => {
    const handler = createMainQuestCommandHandler({
      db,
      ai: { suggest: async () => ({ ok: true, draft: draft() }) },
    });

    const help = await handler.handle({ kind: "help", input: "" }, USER);
    expect(help).toContain("**Main Quest commands**");
    expect(help).toContain("/main suggest <goal>");
    expect(help).not.toContain("Unknown");

    const unknown = await handler.handle(
      { kind: "help", input: "progress quest-1 lots" },
      USER,
    );
    expect(unknown).toContain("Unknown Main Quest command: `progress quest-1 lots`");
    expect(unknown).toContain("**Main Quest commands**");
  });
