...
export const MAIN_COMMAND_HELP = [
  "**Main Quest commands**",
  "- `/main suggest <goal>` — draft a Main Quest from a goal",
  "- `/main accept` — accept the pending draft",
  "- `/main reject` — discard the pending draft",
  "- `/main list` — list active Main Quests",
  "- `/main progress <id> <amount>` — log progress",
  "- `/main complete <id>` — complete a quest",
  "- `/main archive <id>` — archive a quest",
].join("\n");
    ...
    async handle(command, userId) {
      if (command.kind === "help") {
        return command.input
          ? `Unknown Main Quest command: \`${command.input}\`\n\n${MAIN_COMMAND_HELP}`
          : MAIN_COMMAND_HELP;
      }
