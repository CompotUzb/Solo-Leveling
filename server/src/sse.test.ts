...
function makeApi(sseHeartbeatMs?: number) {
  ...
  return createApi({ config, discordStatus: () => "skipped", db, sseHeartbeatMs });
}
...
describe("SSE live update stream", () => {
  ...
  it("sends periodic keep-alive comments so idle connections stay open", async () => {
    const api = makeApi(25);
    await api.app.listen({ host: "127.0.0.1", port: 0 });
    const address = api.app.server.address();
    if (!address || typeof address === "string")
      throw new Error("missing test server address");
    ...
    const text = await readUntil(res.body!, ": keep-alive");
    expect(text).toContain("event: connected");
