...
import { ensureDailyDay } from "./dailyQuests.js";
...
describe("API", () => {
  ...
  it("archives Main Quests without deleting them", async () => {
    ...
    const create = await api.app.inject({
      ...
      url: "/api/main-quests",
      payload: {
        title: "Finish Solo Leveling Deployment",
        difficulty: "hard",
        target: 5,
        unit: "tasks",
      },
  ...
  it("records Daily Quest metrics through the dashboard API", async () => {
    const config = loadConfig({
      DISCORD_TOKEN: "fake",
      DISCORD_CLIENT_ID: "client",
      TRACKED_GUILD_ID: "guild",
    ...
    ensureDailyDay(db, "local-user", today, "e");
    const api = createApi({ config, discordStatus: () => "skipped", db });

    const reps = await api.app.inject({
      method: "POST",
      url: "/api/daily/metric",
      payload: { metricKey: "pushups", delta: 5 },
    });
    expect(reps.statusCode).toBe(200);
    expect(
      reps
        .json()
        .quest.metrics.find((metric: { key: string }) => metric.key === "pushups")
        .progress,
    ).toBe(5);

    const lust = await api.app.inject({
      method: "POST",
      url: "/api/daily/metric",
      payload: { metricKey: "lusted", answer: "no" },
    });
    expect(lust.statusCode).toBe(200);
    expect(
      lust
        .json()
        .quest.metrics.find((metric: { key: string }) => metric.key === "lusted")
        .done,
    ).toBe(true);
    await api.close();
  });
