...
export const DAILY_METRICS = [
  ...
  { key: "lusted", label: "Lusted?", unit: "yes/no" },
] as const;
...
export const DAILY_LUST_CLEAR_STAT_GAINS = [
  { statKey: "intelligence", delta: 1 },
] satisfies StatGain[];
export const DAILY_LUSTED_PENALTY_XP = 50;
export const DAILY_LUSTED_PENALTY_STAT_GAINS = [
  { statKey: "intelligence", delta: -1 },
] satisfies StatGain[];
...
export function getDailyQuest(
  ...
  const lustCheck = metrics.find((m) => m.key === "lusted");
  const requiresLustCheck = Boolean(lustCheck) || day.status === "active";
  const lustCheckDone = requiresLustCheck ? Boolean(lustCheck?.done) : true;
  const completedCount =
    required.filter((m) => m.done).length +
    Number(cardioDone) +
    Number(mentalDone) +
    Number(lustCheckDone && requiresLustCheck);
  const criteriaMet =
    required.every((m) => m.done) && cardioDone && mentalDone && lustCheckDone;
  const complete = day.status === "completed" || (day.status === "active" && criteriaMet);
  return {
    id: day.id,
    date: localDate,
    ...
    complete,
    completedCount,
    totalCount: 6 + Number(requiresLustCheck),
...
export interface LogMetricResult {
  ...
  penalty: DailyLustPenaltyResult | null;
}

export interface DailyLustPenaltyResult {
  quest: DailyQuestView;
  xpAwarded: number;
  statGains: typeof DAILY_LUSTED_PENALTY_STAT_GAINS;
}

/** Record the yes/no lust accountability answer. `No` clears the check; `Yes` fails the day. */
export function logDailyLustAnswer(
  db: Db,
  userId: string,
  localDate: string,
  lusted: boolean,
  ...
  const row = db
    .prepare(
      "select target,progress from daily_quest_metrics where user_id=? and local_date=? and metric_key='lusted'",
    )
    .get(userId, localDate) as
    { target: number; progress: number } | undefined;
  if (!row) throw new Error(`no daily lust check for ${localDate}`);
  ...
  if (!lusted) {
    const changed = row.progress !== 1;
    db.prepare(
      "update daily_quest_metrics set progress=1, updated_at=? where user_id=? and local_date=? and metric_key='lusted'",
    ).run(now, userId, localDate);
    if (changed) {
      applyStatGains(
        db,
        {
          userId,
          gains: DAILY_LUST_CLEAR_STAT_GAINS,
          reason: "daily_lust_check_clear",
          source: "daily_quest",
          sourceId: localDate,
        },
        clock.genId ? { now: () => now, genId: clock.genId } : { now: () => now },
    ...
    const completion = quest.complete
      ? completeDailyDay(db, userId, localDate, hooks)
      : null;
    return {
      quest: completion?.quest ?? quest,
      completion,
      penalty: null,
    };
  }
  ...
  const run = db.transaction(() => {
    ...
    db.prepare(
      `update daily_quest_days
          set status='failed', evaluated=1, completed_at=null, rewards_granted=0, updated_at=?
        where user_id=? and local_date=? and status='active'`,
    ).run(now, userId, localDate);
    db.prepare(
      `update daily_quest_state
          set current_streak=0,
              ...
              updated_at=?
        where user_id=?`,
    ).run("Lusted during the daily quest", now, now, userId);
  });
  run();
  ...
  const award = awardXp(
    ...
    {
      userId,
      amount: -DAILY_LUSTED_PENALTY_XP,
      reason: "daily_lust_check_failed",
      source: "daily_quest",
      sourceId: localDate,
    },
    clock.genId ? { now: () => now, genId: clock.genId } : { now: () => now },
  ...
  applyStatGains(
    ...
    {
      userId,
      gains: DAILY_LUSTED_PENALTY_STAT_GAINS,
      reason: "daily_lust_check_failed",
      source: "daily_quest",
      sourceId: localDate,
    },
  ...
  hooks.notify?.({
    ...
    body: `Lusted: yes. ${award.xpAwarded} XP. Daily streak reset to 0.`,
    metadata: {
      date: localDate,
      source: "daily_lust_check",
      xpAwarded: award.xpAwarded,
    },
  });
  return {
    ...
    penalty: {
      quest,
      xpAwarded: award.xpAwarded,
      statGains: DAILY_LUSTED_PENALTY_STAT_GAINS,
    },
  };
}
...
export function logDailyMetric(
  ...
  if (metricKey === "lusted") {
    throw new Error("lusted metric accepts only yes/no answers");
  }
  const row = db
    .prepare(
  ...
  return { quest: completion?.quest ?? quest, completion, penalty: null };
}
