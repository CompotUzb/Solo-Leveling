...
export function parseDailyProgress(content: string): ParsedDailyMetric[] {
  ...
  const lustAnswer = normalized.match(
    /^(?:(?:lusted\?|lusted|lust)\s*[:=-]?\s*)?(yes|no)$/,
  );
  if (lustAnswer) addLustAnswer(matches, lustAnswer[1] as "yes" | "no", content.trim());
  ...
  for (const match of content.matchAll(
    /\b(\d+(?:\.\d+)?)\s*(?:m|min|mins|minute|minutes?)\s+(?:studied|study|studying|focused|focus|focusing|meditated|meditate|meditating)\b/gi,
  )) {
    add(matches, "mental_minutes", Number(match[1]), match[0]);
  }
  for (const match of content.matchAll(
  ...
  for (const match of content.matchAll(
    /\b(\d+(?:\.\d+)?)\s*pages?\s+(?:read|reading)\b/gi,
  )) {
    add(matches, "mental_pages", Number(match[1]), match[0]);
  }
  ...
  for (const match of matches) {
    if (match.metricKey === "lusted") {
      totals.set(match.metricKey, match);
      continue;
    }
...
function lustCheckLine(quest: DailyQuestView | null | undefined): string {
  const metric = quest?.metrics.find((item) => item.key === "lusted");
  const answeredYes = metric != null && metric.progress < 0;
  const answer = answeredYes ? "Yes" : metric?.done ? "No" : "Unanswered";
  const marker = answeredYes ? "❌" : metric?.done ? "✅" : "⬜";
  return `- ${marker} **Lusted?:** \`${answer}\``;
}
...
/** In-thread reply posted the moment a logged message completes the whole checklist. */
export function formatDailyCompletionReply(
  completion: CompleteDailyResult,
): string {
  const gains = completion.statGains
    .map((gain) => `${gain.statKey} +${gain.delta}`)
    .join(" · ");
  return [
    "**🎉 DAILY QUEST COMPLETE**",
    `Reward: \`+${completion.xpAwarded} XP\` · ${gains}`,
    `🎁 Loot: ${completion.lootBoxes.map((box) => box.rarity).join(", ")} box`,
    ...(completion.leveledUp ? ["⬆️ LEVEL UP!"] : []),
    "Streak advances at the midnight evaluation.",
  ].join("\n");
}

export function formatDailyLustPenaltyReply(
  penalty: DailyLustPenaltyResult,
): string {
  const statLine = penalty.statGains
    .map((gain) => `${gain.statKey} ${gain.delta}`)
    .join(" | ");
  return [
    "**DAILY LUST CHECK FAILED**",
    `Penalty: ${penalty.xpAwarded} XP | ${statLine}`,
...
export async function createDailyQuestForDate(input: {
  ...
  if (
    !input.ensurePublished &&
    quest.discordParentMessageId &&
  ...
  return {
    created:
      quest.discordParentMessageId !== published.parentMessageId ||
      quest.discordThreadId !== published.threadId,
...
export function recordDailyThreadMessage(input: {
  ...
  const initialQuest = getActiveDailyQuestByThread(input.db, input.threadId);
  const now = input.now ?? new Date().toISOString();
  if (initialQuest) {
    ensureDailyDay(
      input.db,
      ...
      initialQuest.tier,
      { now: () => now },
      initialQuest.hunterRank,
    );
  }
  const quest =
    getActiveDailyQuestByThread(input.db, input.threadId) ?? initialQuest;
  ...
  let penalty: DailyLustPenaltyResult | null = null;
  ...
  for (const metric of parsed) {
    ...
    const result =
      metric.metricKey === "lusted" && metric.answer
        ? logDailyLustAnswer(
            input.db,
            input.userId,
            quest.date,
