...
TRACKED_CHANNEL_IDS=skip
API_HOST=0.0.0.0
DATABASE_PATH=/app/data/solo-leveling.sqlite
STORE_MESSAGE_CONTENT=false
CONTENT_MAX_CHARS=0
TIMEZONE=Asia/Tashkent
AI_MAIN_QUEST_ENABLED=false
OPENAI_API_KEY=
OPENAI_MODEL=gpt-4o
```

If the host injects `PORT`, the server uses it automatically. Otherwise set `API_PORT=3333`.
...
```text
server/       Fastify API, Discord bot, SQLite access, CLI commands, tests
web/          Vite React dashboard
shared/       shared types and deterministic XP/rank constants
mobile/       native Android blocker client and mobile status UI assets
...
## Android Mobile Blocker

`mobile/` contains a native Android client that syncs with `/api/daily` and blocks configured social apps while the Daily Quest penalty is active. It uses Android Accessibility settings; it does not read SQLite directly. Open `mobile/` in Android Studio, set the server URL, enable `Solo Leveling App Blocker`, and configure the blocked app tab.
